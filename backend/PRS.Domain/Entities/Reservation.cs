...
public class Reservation
{
    ...
    public static Result<Reservation> Create(
        Spot spot, User user, DateTime from, DateTime to)
    {
        if (to <= from)
        {
    ...
    public Result Cancel()
    {
        if (Status is ReservationStatus.Cancelled or ReservationStatus.Expired)
        {
            return Result.Failure(new DomainError(
                        "Reservation.AlreadyCancelled", "Cannot cancel",
    ...
    public Result CheckIn(DateTime at)
    {
        ...
        {
            return Result.Failure(new DomainError(
                        "Reservation.InvalidState", "Cannot check-in",
